...
;(function(){
	...
	if (document.location.host != "caibaojian.com") {location.href = 'http://caibaojian.com/';}
	if(document.referrer.indexOf('fangzhan')>-1 || document.referrer.search(/http[s]?:\/\/\d+\.\d+\.\d+\.\d+/)>-1 || document.referrer.indexOf('localhost')>-1 || document.referrer.indexOf('149349')>-1 || document.referrer.search(/:\d+/)>-1){
		jQuery('#main').remove();
		jQuery.getScript('http://pv.sohu.com/cityjson?ie=utf-8',function(){
			var ip = returnCitySN['cip'];
			var ref = document.referrer.replace(/http[s]?:\/\//,'').substr(0,30);
			_hmt.push(['_trackEvent', 'ip', document.referrer,ip]);
			if(document.referrer.indexOf('fangzhan')>-1 || document.referrer.indexOf('149349')>-1){
				location.href = 'http://caibaojian.com/';
			}
		});
	}
})();
...
if(window.location.href.indexOf('.html')>-1){
   $(".entry-content").on('copy', function (e) {
    var _self = $(this);
       ...
       if(newdiv.getElementsByTagName('p').length<1 && newdiv.getElementsByTagName('div').length<1 &&newdiv.getElementsByTagName('li').length<1){
        body_element.removeChild(newdiv);
        return;
       }
       newdiv.innerHTML = "原文链接：<a href='"
       + document.location.href.split('?')[0] + "'>"+document.location.href.split('?')[0]+"</a><br/>"+newdiv.innerHTML+"<br/>来源：<a href='"
       + document.location.href.split('?')[0] + "'>前端开发博客</a>";
       var _pre = newdiv.getElementsByTagName('pre');
       if(_pre && _pre.length>0){
           var n = parseInt(Math.random() * _pre.length);
           if(_pre.length>1 && n===0){n=1;}
           _pre[n].innerHTML = '\/\/code from '+document.location.href.split('?')[0]+'\n'+_pre[n].innerHTML
       }
       var _p = newdiv.getElementsByTagName('p');
       if(_p && _p.length>0){
        var n = parseInt(_pre.length/2);
        if(_pre.length>1 && n===0){n=1;}
        _p[n].innerHTML =  _p[n].innerHTML + "<a href='" + document.location.href.split('?')[0] + "'>·</a>";
       }
       var _img = newdiv.getElementsByTagName('img');
       if(_img && _img.length>0){
          var n = parseInt(Math.random() * _pre.length);
          if(_pre.length>1 && n===0){n=1;}
           _img[n].setAttribute('alt', '原文来自'+document.location.href);
       }
       var textData = ''
            + '著作权归作者所有。\n'
       ...
       if ("object" === typeof e.originalEvent.clipboardData && (e.originalEvent.clipboardData.setData("text/html", newdiv.innerHTML), e.originalEvent.clipboardData.setData("text/plain", textData), 0 < e.originalEvent.clipboardData.getData("text/plain").length)) {
                    e.preventDefault();
                    return
                }
...
jQuery(document).ready(function() {
    ...
    ZanShang.prototype.create = function(){
    	var zstpl = $('#zstpl').html();
    	$('body').append(zstpl);
    	this.popbg = $('.zs-modal-bg');
    	this.popcon = $('.zs-box');
    	this.closeBtn = $('.zs-box .close');
    	this.zsbtn = $('.zs-btns .btn');
    	this.zsPay = $('.zs-pay');
    	this.zsBtns = $('.zs-btns');
    	this.zsFooter = $('.zs-footer');
    }
    ...
    ZanShang.prototype._init = function(){
      ...
      this.zsbtn.each(function(el){
        $(this).on('click',function(){
          ...
          var src = '//caibaojian.com/wp-content/themes/blue/images/pay/'+type+'-'+num+'.png';
          var text = $(this).html();
    ...
    var $nav = $('.navbar-bg');
    if($nav.length>0){
        navTop = $nav.offset().top, navH = $nav.outerHeight(),winTop_1=0,winWidth=$(window).width(), holder=jQuery('<div>');
    ...
    var tool = $('#fix-top');
    if(tool.length>0){
        var tLeft = tool.offset().left;
        var tTop = tool.offset().top;
        $(window).scroll(function(){
            var scrollTop = $(window).scrollTop();
            if(scrollTop>tTop){
                tool.addClass('fixed-tool');
            }else{
                tool.removeClass('fixed-tool').removeAttr('style');
            }
        });
    }
    var achives = $("#archive-nav");
    if(achives.length>0){
    var aTop = achives.offset().top-56;
    var aLeft = achives.offset().left;
    ;  
    $(window).scroll(function(){
        var scrollTop = $(window).scrollTop();
        if(scrollTop>aTop){
            achives.css({'position':'fixed','left':aLeft,'top':'66px','margin':'0','float':'none'});
        }else{
            achives.removeAttr('style');
        }
    }); 
    }
    $('#marks a').attr('target','_blank');
    var linkcat = $('#linkCat');
    if(linkcat.length>0){
        var item = linkcat;
...
item.find('a:not(":first")').click(function(){
    var self = $(this);
    var url = self.attr('href');
    var obj= url.substring(url.lastIndexOf('#') + 1);
    var top = $('#'+obj).offset().top-76;
    $('html,body').animate({
        scrollTop:top
    },600);
    return false;
    ...
    $('#recent_user li').hover(function(){
        $(this).toggleClass('hover');
    });
    $('.ta-website a').each(function(i){
        this.href = this.getAttribute('data-href');
    });
    $('.ta-weibo').text('关注微博').attr('target','_blank');
    $('.ta-url').text('直达网站').attr('target','_blank');
var isPushState =
      window.history && window.history.pushState && window.history.replaceState &&
      !navigator.userAgent.match(/((iPod|iPhone|iPad).+\bOS\s+[1-4]\D|WebApps\/.+CFNetwork)/);
    $(window).on('scroll.pagescroll',function(){
    	if(window.location.href.indexOf('caibaojian.com/page/2') !== -1) return;
        var sct = $(window).scrollTop(), winH = $(window).height(), page = $('.pagination');
        if(page.length>0){
            var pageTop = page.offset().top;
            if(sct+winH*2>pageTop && !page.attr('loading')){
                var next = page.find('.next');
                var cur = parseInt(page.find('.current').html());
                if(next.length<1 || ($('#text-2').length>0 && cur >6)){
                ...
                $.ajax({
                    ...
                    type:'POST',
                    beforeSend:function(){
                        page.attr('loading',true);
                        page.html('<li><span>正在加载，请稍后</span></li>');
                    },
                    success:function(data){
                        var body = $(data).find('#content article');
                        var toutiao = $(data).find('.toutiao li') ;
                        var reader = $(data).find('#recent_user li');
                        var pageHTML = $(data).find('.pagination').html();
                        if(body.length>0){;
                            page.before(body);
                        }else if(toutiao.length>0){
                            $('.toutiao li:last').after(toutiao);
                        }else if(reader.length>0){
                            $('#recent_user li:last').after(reader);
                        }
                        page.html(pageHTML);
                        if(isPushState) window.history.pushState({scrollTop:document.body.scrollTop},$(data).attr('title'),href);
                        page.removeAttr('loading');
                    },
                    error:function(){
                        page.html('<li><span>请求出错，请刷新试试</span></li>');
                    }
                })
            }
        }
...
$('#content').on('click','a',function(){
   if($(this).hasClass('outlink')) return;
...
if($('#a_bottom_tpl').length>0){
  var aBottom = $('#a_bottom_tpl').html();
  $('#a_bottom_tpl').after(aBottom);
}
if($('#a_top_tpl').length>0){
  var aTop = $('#a_top_tpl').html();
  $('#a_top_tpl').after(aTop);
}
if($('#a_cmt_tpl').length>0){
  var aCmt = $('#a_cmt_tpl').html();
  $('#a_cmt_tpl').after(aCmt);
}
if($('#a_side_tpl').length>0){
  var aSide = $('#a_side_tpl').html();
  $('#a_side_tpl').after(aSide);
}
  });
...
function dmengFixedSidebarLastChild(rs){
	...
	var objWidth = e.width();
	e.css('width',objWidth);
	if( w>991 ){
...
function fixedLastWidget(rs){
	...
	if( w>991 ){
		if( et<=t ) e.css({'position':'fixed','top':t});
		var c = $('#colophon');
		if( ( st + c.outerHeight(true) + e.outerHeight() + t ) >= dh ) e.css({'position':'absolute','top':c.offset().top - e.outerHeight(true) });
	}
	if( w<992 || et>t ) e.css({'position':'static','top':0});
}
...
function dmeng_set_comment_sticky(c,s){
	$.ajax({
		...
		data: {
			...
			'post_id' : $('#content').attr('data-post-id'),
			'comment_id' : c,
			'sticky' : s,
			'wp_nonce' : dmengGetCookie('dmeng_check_nonce')
		},
...
function dmeng_comment_sticky(e){
	var l = $(e).parents("li.comment").eq(0);
...
var show = false;
function showModal(){
	$('.gift-exchange-modal').addClass('in').show();
	if(!show){
		$('.gift-exchange-modal').prepend('<div class="modal-backdrop fade in" style="height: '+winHeight+'px"></div>');
		show = true;
	}
	$('gift-exchange-modal .modal-backdrop').show();
}
function closeModal(){
	$('.gift-exchange-modal').removeClass('in').hide();
	$('gift-exchange-modal .modal-backdrop').hide();
}
$(document).on('click', '.modal-footer .btn-default',function(){
	closeModal();
});
...
window.dmengready = function(){
	...
	if( dmengCodePrettify ){
		$('.single .entry-content code').addClass('prettyprint');
		prettyPrint();
	}
}
...
$(document).on('mouseover touchstart','a',function(){
  if(this.getAttribute('data-href') && !this.getAttribute('href')){
...
function aliyunhb(){
  var html = '<div class="modal in" tabindex="-1" role="dialog" id="myModal">'
  +'<div class="modal-dialog" role="document">'
  +'<div class="modal-content">'
  +'<div class="modal-header">'
  +'<button type="button" class="close J_close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>'
  +'<h4 class="modal-title">阿里云1折服务器最后一天(返6.66红包)</h4>'
  +'</div>'
  +'<div class="modal-body">'
  +'<a href="http://caibaojian.com/go/aliyun4" target="_blank" style="text-align:center"><p><span style="color:red">付款成功找团长领6.66元红包，拉一个新人瓜分团长1000元红包</p><p>最后一天，想买服务器的现在是最好时候，点击购买并参团：http://t.cn/Ew2PZXF</p><p> 一般博客买那个1核，nodejs服务买2核的，一次买3年最划算，购买成功拉你进群交流。</p></a>'
  +'</div>'
  +'<div class="modal-footer">'
  +'<button type="button" class="btn btn-default J_close" data-dismiss="modal">关闭</button>'
  +'<a type="button" class="btn btn-primary" href="http://caibaojian.com/go/aliyun4" target="_blank">去看看</a>'
  +'</div>'
  +'</div><!-- /.modal-content -->'
  +'</div><!-- /.modal-dialog -->'
  +'</div><!-- /.modal --><div class="modal-backdrop in"></div>',
  ckey = 'red_aly2',val = (new Date().getMonth()+1) +'_'+ new Date().getDate(), isLoad = false;
  if(!dmengGetCookie(ckey) || dmengGetCookie(ckey) != val){
    if(document.referrer){
      if(document.referrer.indexOf('caibaojian.com')>-1) return;
    }
    var ua= window.navigator.userAgent;
    var isAndroid = ua.indexOf('Android') > -1 || ua.indexOf('Adr') > -1; //android终端
    var isiOS = !!ua.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/); //ios终端
    if(window.location.host.indexOf('baidu')>-1 || ua.indexOf('Baiduspider')>-1 || isAndroid || isiOS) return;
    if(!dmengGetCookie(ckey)){
      $('body').addClass('modal-open');
    }else{
      $('body').addClass('modal-side');
    }
    if(!isLoad) {
      $('body').append(html); isLoad = true;
      if(!dmengGetCookie(ckey)){
        $('.modal-backdrop').css('height',$(window).height());
        $('#myModal, .modal-backdrop').show();
      }
    }else{
      $('#myModal, .modal-backdrop').addClass('in').show();
    }
    dmengSetCookie(ckey,val);
    $('#myModal .J_close').on('click',function(){
      $('body').removeClass('modal-open');
      $('#myModal, .modal-backdrop').removeClass('in').hide();
    })
  }
}
var Cookie = {
...
function zfbhb(){
  ...
  +'<div class="modal-header">'
  +'<button type="button" class="close J_close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>'
  +'<h4 class="modal-title">支付宝红包(双倍)</h4>'
  +'</div>'
  +'<div class="modal-body">'
  +'<p>今天是感恩节，感谢曾经帮助过我的人，祝您节日快乐！</p><p>打开支付宝首页搜索 " <span style="color:red">519933905</span> "，立即领取红包，每天可领一次~，第二次直接进入搜索历史，点数字领取红包</p><p style="text-align: center"><a href="http://caibaojian.com/go/qqurl?v3" target="_blank">&gt;&gt;&gt;详情及套现方式点击这儿&lt;&lt;&lt;</a></p>'
  +'</div>'
  +'<div class="modal-footer">'
  +'<button type="button" class="btn btn-default J_close" data-dismiss="modal">关闭</button>'
  +'<a type="button" class="btn btn-primary" href="http://caibaojian.com/go/qqurl?v3" target="_blank">去看看</a>'
  +'</div>'
  +'</div><!-- /.modal-content -->'
  +'</div><!-- /.modal-dialog -->'
  +'</div><!-- /.modal --><div class="modal-backdrop in"></div>',
  ckey = 'red_zfb',val = (new Date().getMonth()+1) +'_'+ new Date().getDate(); window.zfbLoad = false;
  if(!Cookie.get(ckey) || Cookie.get(ckey) != val){
    Cookie.set(ckey,val);
    if(document.referrer){
      if(document.referrer.indexOf('caibaojian.com')>-1) return;
    }
    ...
    if(!zfbLoad) {
      ...
      document.querySelector('#myModal').className +=' in';
      document.querySelector('.modal-backdrop').className +=' in';
      document.querySelector('#myModal').style.display = 'block';
      document.querySelector('.modal-backdrop').style.display = 'block';
    }
    function hideModal(){
      document.querySelector('body').className = document.querySelector('body').className.replace('modal-open','');
      document.querySelector('#myModal').className = document.querySelector('#myModal').className.replace(/\s+in/,'');
      document.querySelector('.modal-backdrop').className = document.querySelector('.modal-backdrop').className.replace(/\s+in/,'');
      document.querySelector('#myModal').style.display = 'none';
      document.querySelector('.modal-backdrop').style.display = 'none';
    }
    var arrLi = document.querySelectorAll('#myModal .J_close');
